...
**Design → Artwork → Adjust artwork** stays visible beneath the artwork choice. Choose **Placement → Behind content** to put any built-in artwork or custom drawing on a separate background layer. **Artwork size** runs from 25–400%: 100% fits the whole artwork; larger values crop at the card edges. **Position** moves it and **Opacity** (0–100%) controls its strength. **Opacity fade** offers **Uniform**, **Linear**, and **Radial**. Linear fades rotate through 0–360°; radial fades have horizontal and vertical center controls. **Reverse fade** swaps which end or area remains visible. Opacity is the maximum strength before the fade. The photo and selectable text remain separate, and sizing the artwork does not resize the card. Existing drafts keep their placement until you change it.

Side motifs—including **Dots**, **Orbits**, **Grid**, and custom drawings—have **Size** (25–100%), **Horizontal position**, and **Vertical position** (0–100%). Size is a percentage of the largest artwork that fits the current side area; it does not change the signature dimensions. Reduce Size to make room to move an artwork along an axis that already fits exactly. An unavailable axis is disabled.

The six abstract studies use artwork composed for the whole Wide or Tall signature. With **Adjust artwork → Placement → Template default**, their shapes flow across the card while names and contacts remain separate text. Choose **Across signature** explicitly or **Side detail** for the smaller motif. Flowing artwork has **Scale** (25–400%), **Horizontal position**, and **Vertical position** (0–100%) controls. Position controls enable when the scaled image has room to move along that axis. All patterns and custom drawn grids can also use **Behind content**.
...
**Size & spacing** offers width (280–420 px, or 220–420 for a horizontal Single card) and height (180–320 px) controls. Left & right padding can override the card padding from 0–48 px without changing the top and bottom. Choose Follow card padding to restore the template/card inset. Reduce the side padding, then the width to trim the outer card without shrinking the content. For Single card, width is the full card width vertically or the width of each column horizontally; Minimum height grows as needed to fit your details. For Front & back, width and height size each face. The canvas footer shows the actual exported dimensions, shared by the preview, copied HTML, and PNG export.
