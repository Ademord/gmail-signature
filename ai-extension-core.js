(function (root, factory) {
  ...
  var artworkKeys = ['pattern', 'customPattern', 'artworkPlacement', 'artworkScale', 'artworkOpacity', 'artworkPositionX', 'artworkPositionY', 'motifScale', 'motifPositionX', 'motifPositionY'];
  ...
  function strictField(key, value) {
    ...
    if (['width','height','cardGap','portraitSize','artworkScale','artworkOpacity','artworkPositionX','artworkPositionY','motifScale','motifPositionX','motifPositionY'].includes(key)) {
      var bounds = key === 'width' ? [280,420] : key === 'height' ? [180,320] : key === 'cardGap' ? [0,60] : key === 'artworkScale' ? [25,400] : key === 'motifScale' ? [25,100] : (key === 'artworkOpacity' || key.startsWith('artworkPosition') || key.startsWith('motifPosition')) ? [0,100] : [40,96];
    ...
    var choices = key === 'design' ? core.designs.map(function (item) { return item.id; }).concat('custom') :
      ...
      key === 'artworkPlacement' ? ['auto','motif','flow','background'] : key === 'portraitShape' ? ['circle','rounded','square'] : iconKeys.includes(key) ? Object.keys(core.icons) : null;
