(function (root, factory) {
  ...
  var formatKeys = ['nameLayout', 'nameFontSize', 'titleFontSize', 'subtitleFontSize', 'contactFontSize', 'footerFontSize', 'lineSpacing', 'textSpacing', 'contactSpacing', 'sectionSpacing', 'contentPadding','sidePadding', 'singleArrangement', 'contactLayout', 'contactFont', 'footerVisible', 'contactSeparator'].concat(visibilityKeys);
  ...
  var formatNumbers = Object.freeze({nameFontSize:[12,40,0], titleFontSize:[8,24,0], subtitleFontSize:[8,24,0], contactFontSize:[8,24,0], footerFontSize:[8,24,0], lineSpacing:[80,200], textSpacing:[0,200], contactSpacing:[0,200], sectionSpacing:[0,200], contentPadding:[-1,48], sidePadding:[-1,48]});
  var compactFallback = Object.freeze({cardFormat:'single', layout:'paired', singleArrangement:'rows', width:280, height:180, contentPadding:24, sidePadding:-1, portraitSize:96, nameLayout:'single', nameFontSize:22, titleFontSize:13, subtitleFontSize:13, contactFontSize:13, contactFont:'sans', contactLayout:'inline', contactSeparator:'bar', lineSpacing:100, textSpacing:100, contactSpacing:100, sectionSpacing:150, footerVisible:'hide'});
  ...
  function strictField(key, value) {
    ...
    if (Object.hasOwn(formatNumbers, key)) {
      ...
      if (typeof value !== 'number' || !Number.isInteger(value) || (value !== range[2] && (value < range[0] || value > range[1]))) {
        fail(key + ' must be ' + (range.length > 2 ? range[2] + ' (template size) or ' : '') + 'a whole number from ' + range[0] + ((key === 'contentPadding' || key === 'sidePadding') ? ' (template padding)' : '') + ' to ' + range[1] + '.');
      }
      return value;
    }
    if (['width','height','cardGap','portraitSize','artworkScale','artworkOpacity','artworkFadeAngle','artworkFadeX','artworkFadeY','artworkPositionX','artworkPositionY','motifScale','motifPositionX','motifPositionY'].includes(key)) {
      var bounds = key === 'width' ? [220,420] : key === 'height' ? [180,320] : key === 'cardGap' ? [0,60] : key === 'artworkFadeAngle' ? [0,360] : key === 'artworkScale' ? [25,400] : key === 'motifScale' ? [25,100] : (['artworkOpacity','artworkFadeX','artworkFadeY'].includes(key) || key.startsWith('artworkPosition') || key.startsWith('motifPosition')) ? [0,100] : [40,96];
  ...
  function buildPrompt(section, draft, settings) {
    ...
    if (keys.some(function (key) { return colorKeys.includes(key); })) lines.push('Colors use six-digit hex values (#rrggbb). frontBackground is the light/front field, backBackground is the second field, accent is decorative color. Explicit single-card format uses frontBackground for the whole card and preserves backBackground without displaying it. Prefer coordinated colors and readable contrast; the renderer chooses readable text automatically.');
