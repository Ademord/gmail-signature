...
test('46-field sessions preserve independent side and flowing settings, importing them only with Design',()=>{
  assert.equal(Object.keys(core.defaults).length,46);
  ...
  const old=Object.fromEntries(Object.entries(incoming.draft).filter(([key])=>!keys.includes(key)&&!['cardGap','cardFormat','artworkOpacity','artworkFade','artworkFadeAngle','artworkFadeDirection','artworkFadeX','artworkFadeY'].includes(key)));assert.equal(Object.keys(old).length,35);
